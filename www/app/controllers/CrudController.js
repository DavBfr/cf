function CrudService($http, service) {
	...
	this.get_list = function (page, onsuccess, onerror) {
		if (page<0) {
			onsuccess && onsuccess([], 200);
			return;
		}
		$http.get(service_url + "?p="+page).success(function (data, status) {
			if (data.success)
	...
	this.get_count = function (onsuccess, onerror) {
		$http.get(service_url + "/count").success(function (data, status) {
			if (data.success)
...
function CrudController($scope, $timeout, $location, $route, CrudService, NotificationFactory) {
	...
	this.get_list = function() {
		CrudService.get_count(function (count) {
	...
	$scope.setPage = function(num) {
		...
		$scope.page = num;
		CrudService.get_list($scope.page, function (data) {
