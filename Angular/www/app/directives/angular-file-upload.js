...
(function () {
	...
	let angularFileUpload = angular.module('angularFileUpload', []);
	...
	angularFileUpload.service('$upload', ['$http', '$q', '$timeout', function ($http, $q, $timeout) {
		...
		this.upload = function (config) {
			...
			let origTransformRequest = config.transformRequest;
			let origData = config.data;
			config.transformRequest = function (formData, headerGetter) {
				if (origData) {
					if (config.formDataAppender) {
						for (let key in origData) {
							let val = origData[key];
						...
						for (let key in origData) {
							...
							if (typeof origTransformRequest === 'function') {
								...
								for (let i = 0; i < origTransformRequest.length; i++) {
									let transformFn = origTransformRequest[i];
...
//for backward compatibility
	angularFileUpload.directive('ngFileDropAvailable', ['$parse', '$timeout', function ($parse, $timeout) {
		return function (scope, elem, attr) {
			if (dropAvailable()) {
				let fn = $parse(attr['ngFileDropAvailable']);
				$timeout(function () {
					fn(scope);
				});
			}
	...
	function handleDrop(scope, elem, attr, ngModel, $parse, $timeout, $location) {
		...
		let stopPropagation = scope.stopPropagation();
		let dragOverDelay = 1;
		let accept = scope.accept() || attr['accept'] || attr['ngAccept'];
		let regexp = accept ? new RegExp(globStringToRegex(accept)) : null;
		...
		function calculateDragOverClass(scope, attr, evt) {
			...
			let clazz = scope.dragOverClass({$event: evt});
			if (clazz) {
		...
		function extractFiles(evt, callback, allowDir, multiple) {
			let files = [], rejFiles = [], items = evt.dataTransfer.items;
